...
int main(int argc, char** argv) {
    ...
    float light_angle = 0.;
    float light_scale = 1.;
    float light_scale_step = 0.01;
    glm::vec4 light_pos = glm::vec4(2,1,1,1);
    ...
    while(!done) {
        ...
        // light
        if (light_scale > 1.5 || light_scale < 0.5) {
            light_scale_step = -light_scale_step;
        }
        light_scale += light_scale_step;
        light_angle += M_PI / 128.;
        //std::cout << light_scale << ", " <<  light_scale_step << std::endl;
        ...
        glBindVertexArray(vao); 
            ...
            auto light_pos_r = glm::scale(glm::rotate(MVMatrix, light_angle, glm::vec3(0,0,1))
                                 , glm::vec3(light_scale,light_scale,light_scale)) * light_pos;
            glUniform3fv(program.uLightPos_vs, 1, glm::value_ptr(light_pos_r));
